...
struct LCSettingsView: View {
    ...
    func importCertificateFromFeather() async {
        ...
        comps.scheme = "feather"
        comps.host = "export-certificate"
        comps.queryItems = [URLQueryItem(name: "callback", value: callbackURL)]

        guard let url = comps.url else {
            errorInfo = "Failed to build Feather certificate request URL."
            errorShow = true
            return
        }
